...
#define LINUX_SYSCALL(linux_nr, leb_nr) [linux_nr] = (leb_nr) + 1
    ...
    LINUX_SYSCALL(285, SYSCALL_SCHED_GETAFFINITY),
    LINUX_SYSCALL(294, SYSCALL_SCHED_SETAFFINITY),
...
static int syscall_needs_expanded_stack(int num) {
    ...
    if (num == SYSCALL_CLONE || num == SYSCALL_VFORK ||
        num == SYSCALL_CLONE3) return 1;
