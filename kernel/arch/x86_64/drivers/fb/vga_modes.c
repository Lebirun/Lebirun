...
static uint64_t cirrus_fb_base = 0;
static uint64_t cirrus_vram = 0;
...
static uint64_t cirrus_detect_vram(void) {
    uint8_t sr0f;
    uint64_t size;
...
static void vga_set_crtc_timing(uint16_t width, uint16_t height) {
    uint64_t htotal;
    uint64_t hdispend;
    uint64_t hblankstart;
    uint64_t hblankend;
    uint64_t hsyncstart;
    uint64_t hsyncend;
    uint64_t vtotal;
    uint64_t vdispend;
    uint64_t vblankstart;
    uint64_t vblankend;
    uint64_t vsyncstart;
    uint64_t vsyncend;
    uint8_t overflow;
...
int vga_set_mode(uint16_t width, uint16_t height, uint16_t bpp, uint64_t *out_pitch) {
    ...
    pitch = (uint64_t)width * (uint64_t)(bpp / 8);
    pitch = (pitch + 63) & ~63u;
    needed = pitch * (uint64_t)height;
    if (cirrus_vram && needed > cirrus_vram) return -4;

    offset_val = pitch / 8;
    if (offset_val > 0x1FF) return -3;

    vtotal_check = (uint64_t)height + 8;
