...
void ipv67_receive_on_port_locked(uint16_t local_port, uint32_t src_ipv4, uint16_t src_port, const uint8_t *packet, uint64_t len) {
    ...
    if (!packet || len < sizeof(ipv67_header_t) || len > 65535) {
    ...
    if (hdr.type == IPV67_TYPE_PEER_REQ || hdr.type == IPV67_TYPE_PEER_ACK || hdr.type == IPV67_TYPE_ROUTE_ADV) {
        ipv67_apply_route_adv(IPV67_PEER_IPV4, src_ipv4, NULL, src_port, payload, payload_len);
    }
    ...
    if (ipv67_self_set && !ipv67_addr_eq(&dst_addr, &ipv67_self)) {
        ...
        if (fwd) {
            ...
            for (i = 0; i < ipv67_current->peer_cap; i++) {
                ...
                if (ipv67_peers[i].family == IPV67_PEER_IPV4 && ipv67_peers[i].ipv4 == src_ipv4 && ipv67_peers[i].port == src_port) continue;
