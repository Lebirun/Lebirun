...
typedef struct {
    int in_use;
    epoll_fd_entry_t fds[MAX_EPOLL_EVENTS];
    int fd_count;
    int flags;
    mutex_t lock;
} epoll_instance_t;
...
static futex_t futexes[MAX_FUTEXES];
static mutex_t futex_lock;

static eventfd_instance_t eventfds[MAX_EVENTFDS];
...
static futex_t *find_futex(uint32_t *addr) {
    for (int i = 0; i < MAX_FUTEXES; i++) {
...
static futex_t *alloc_futex(uint32_t *addr) {
    for (int i = 0; i < MAX_FUTEXES; i++) {
        if (!futexes[i].in_use) {
...
static int sys_epoll_ctl(int epfd, const char *op_ptr, int fd) {
    ...
    switch (op) {
        case EPOLL_CTL_ADD:
            ...
            if (ep->fd_count >= MAX_EPOLL_EVENTS) {
                mutex_unlock(&ep->lock);
                return -ENOSPC;
            }
...
static int sys_epoll_wait(int epfd, const char *events_ptr, int maxevents) {
    ...
    for (int i = 0; i < ep->fd_count && count < maxevents; i++) {
        ...
        uint32_t revents = 0;
        int fd = ep->fds[i].fd;
        uint32_t wanted = ep->fds[i].events;
...
static int sys_futex(int *uaddr, const char *op_ptr, int val) {
    ...
    switch (cmd) {
        case FUTEX_WAIT: {
            ...
            futex_t *f = find_futex((uint32_t *)uaddr);
            if (!f) {
                f = alloc_futex((uint32_t *)uaddr);
                if (!f) {
