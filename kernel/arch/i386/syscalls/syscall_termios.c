...
static int user_range_mapped(uint32_t addr, uint32_t size) {
    ...
    if (addr < 0x1000 || end >= 0xC0000000) return 0;

    uint32_t p = addr & ~0xFFFu;
    uint32_t pend = end & ~0xFFFu;
    for (;;) {
        if (vmm_get_phys_in_pd(current_task->cr3, p) == 0) return 0;
        if (p == pend) break;
        if (p > 0xFFFFF000u) return 0;
        p += 0x1000u;
    }
    return 1;
}
...
static int ioctl_fcntl_basic_compat(int fd, int cmd, int arg) {
    ...
    switch (cmd) {
        ...
        case 4: 
            current_task->fds[fd].flags = (current_task->fds[fd].flags & 1) | ((uint32_t)arg & ~1u);
            return 0;
        default:
            return -EINVAL;
    }
...
static int sys_ioctl(int fd, const char *request_ptr, int arg) {
    ...
    switch (request) {
        ...
        case TIOCGETA:
            if (tty_id < 0) return -ENOTTY;
            if (!arg || !user_range_mapped((uint32_t)arg, sizeof(struct kernel_termios))) return -EFAULT;
