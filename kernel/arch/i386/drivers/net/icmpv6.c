...
void icmpv6_receive(netif_t *netif, ipv6_addr_t *src, uint8_t *data, uint32_t len) {
...
int icmpv6_send_echo_request(netif_t *netif, ipv6_addr_t dest, uint16_t id, uint16_t seq, uint8_t *data, uint32_t len) {
    if (!netif) return -1;

    uint32_t icmp_len = 8 + len;
