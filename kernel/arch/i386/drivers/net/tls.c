...
static int wolf_recv_cb(WOLFSSL *ssl, char *buf, int sz, void *ctx)
{
    ...
    n = tcp_recv(tcp, (uint8_t *)buf, (uint32_t)sz, 15000);
...
static int wolf_send_cb(WOLFSSL *ssl, char *buf, int sz, void *ctx)
{
    ...
    (void)ssl;
    tcp = (tcp_socket_t *)ctx;
    r = tcp_send(tcp, (uint8_t *)buf, (uint32_t)sz);
...
int tls_recv(tls_conn_t *conn, uint8_t *buf, uint32_t len, uint32_t timeout_ms)
{
    int r;
