...
static inline uint32_t pci_config_read32(uint8_t bus, uint8_t slot, uint8_t func, uint8_t offset) {
    ...
    data |= (uint32_t)inb(PCI_CONFIG_DATA + 1) << 8;
    data |= (uint32_t)inb(PCI_CONFIG_DATA + 2) << 16;
    data |= (uint32_t)inb(PCI_CONFIG_DATA + 3) << 24;
    return data;
}

static inline void pci_config_write32(uint8_t bus, uint8_t slot, uint8_t func, uint8_t offset, uint32_t value) {
    uint32_t address = (uint32_t)((bus << 16) | (slot << 11) |
                      (func << 8) | (offset & 0xFC) | 0x80000000);
    outb(PCI_CONFIG_ADDRESS, address & 0xFF);
    outb(PCI_CONFIG_ADDRESS + 1, (address >> 8) & 0xFF);
...
static inline uint32_t ahci_hba_read(uint32_t base, uint32_t reg) {
    return mmio_read32(base + reg);
}

static inline void ahci_hba_write(uint32_t base, uint32_t reg, uint32_t value) {
    mmio_write32(base + reg, value);
}

static inline uint32_t ahci_port_read(ahci_port_t *port, uint32_t reg) {
    return mmio_read32(port->port_base + reg);
}

static inline void ahci_port_write(ahci_port_t *port, uint32_t reg, uint32_t value) {
...
static ahci_dev_type_t ahci_check_type(ahci_port_t *port) {
    uint32_t ssts = ahci_port_read(port, AHCI_PxSSTS);
    ...
    uint32_t sig = ahci_port_read(port, AHCI_PxSIG);
    switch (sig) {
        case SATA_SIG_ATAPI:
...
static void ahci_stop_cmd(ahci_port_t *port) {
    uint32_t cmd = ahci_port_read(port, AHCI_PxCMD);
...
static int ahci_find_slot(ahci_port_t *port) {
    uint32_t slots = ahci_port_read(port, AHCI_PxSACT) | ahci_port_read(port, AHCI_PxCI);
    for (uint32_t i = 0; i < g_ahci_controller.num_cmd_slots; i++) {
        if ((slots & (1 << i)) == 0)
            return i;
    }
    return -1;
...
static int ahci_wait_cmd(ahci_port_t *port, int slot, uint32_t timeout_ms) {
    uint32_t timeout = timeout_ms;
    
    while (timeout--) {
        uint32_t ci = ahci_port_read(port, AHCI_PxCI);
        if ((ci & (1 << slot)) == 0)
            return 0;
        
        uint32_t is = ahci_port_read(port, AHCI_PxIS);
        if (is & (1 << 30)) { 
...
int ahci_port_init(ahci_port_t *port) {
    ...
    uint32_t cmd_list_phys = pfa_alloc();
    if (!cmd_list_phys) {
        printf("AHCI: Failed to allocate command list for port %u\n", port->port_num);
        return -1;
    }
    
    uint32_t fis_phys = pfa_alloc();
    if (!fis_phys) {
    ...
    uint32_t cmd_table_phys = pfa_alloc();
    if (!cmd_table_phys) {
    ...
    uint32_t cmd_list_virt = 0xF8000000 + (port->port_num * 3 * PAGE_SIZE);
    uint32_t fis_virt = cmd_list_virt + PAGE_SIZE;
    uint32_t cmd_table_virt = fis_virt + PAGE_SIZE;
    ...
    printf("AHCI: Port %u initialized (CLB=0x%08X, FB=0x%08X, CTBA=0x%08X)\n",
...
int ahci_identify(ahci_port_t *port) {
    ...
    uint32_t buf_virt = 0xF9000000 + (port->port_num * PAGE_SIZE);
    ...
    if (port->sector_count == 0) {
        port->sector_count = (uint32_t)identify[60] | ((uint32_t)identify[61] << 16);
    }
...
int ahci_read_sectors(ahci_port_t *port, uint64_t lba, uint32_t count, void *buffer) {
    ...
    uint32_t buf_pages = (count * AHCI_SECTOR_SIZE + PAGE_SIZE - 1) / PAGE_SIZE;
    uint32_t buf_phys = pfa_alloc_contiguous(buf_pages);
    if (!buf_phys) {
        printf("AHCI: Failed to allocate DMA buffer\n");
        return -1;
    }
    
    uint32_t buf_virt = 0xFA000000;
...
int ahci_init(void) {
    ...
    uint32_t abar_phys = g_ahci_controller.abar;
    uint32_t abar_virt = 0xF7100000;
    
    for (uint32_t offset = 0; offset < 0x2000; offset += PAGE_SIZE) {
        vmm_map_page(abar_virt + offset, abar_phys + offset, 0x003);
    }
    
    g_ahci_controller.abar_virt = abar_virt;
    
    uint32_t cap = ahci_hba_read(abar_virt, AHCI_CAP);
    ...
    printf("AHCI: CAP=0x%08X, PI=0x%08X, VS=0x%08X\n", cap, g_ahci_controller.ports_impl, g_ahci_controller.version);
    printf("AHCI: Version %u.%u, %u command slots\n",
           (g_ahci_controller.version >> 16) & 0xFFFF,
           g_ahci_controller.version & 0xFFFF,
           g_ahci_controller.num_cmd_slots);
    
    uint32_t ghc = ahci_hba_read(abar_virt, AHCI_GHC);
    ghc |= AHCI_GHC_AE;
    ahci_hba_write(abar_virt, AHCI_GHC, ghc);
    
    uint32_t ports_impl = g_ahci_controller.ports_impl;
    for (uint32_t i = 0; i < AHCI_MAX_PORTS; i++) {
        if (!(ports_impl & (1 << i)))
            continue;
...
int ahci_port_reset(ahci_port_t *port) {
    ...
    uint32_t sctl = ahci_port_read(port, AHCI_PxSCTL);
    sctl = (sctl & ~0x0F) | 0x01;
...
int ahci_port_recover(ahci_port_t *port) {
    ...
    for (uint32_t i = 0; i < AHCI_CMD_SLOTS; i++) {
...
static int ahci_check_port_error(ahci_port_t *port) {
    uint32_t is = ahci_port_read(port, AHCI_PxIS);
    uint32_t tfd = ahci_port_read(port, AHCI_PxTFD);
    
    if (is & AHCI_PxIS_TFES) {
        port->last_error = AHCI_ERR_TASKFILE;
        port->last_tfd = tfd;
        port->last_serr = ahci_port_read(port, AHCI_PxSERR);
        printf("AHCI: Port %u task file error (TFD=0x%08X, SERR=0x%08X)\n",
    ...
    if (is & (AHCI_PxIS_HBFS | AHCI_PxIS_HBDS)) {
        ...
        port->last_serr = ahci_port_read(port, AHCI_PxSERR);
        printf("AHCI: Port %u DMA error (IS=0x%08X)\n", port->port_num, is);
        return -1;
    }
    ...
    if (is & AHCI_PxIS_IFS) {
        ...
        port->last_serr = ahci_port_read(port, AHCI_PxSERR);
        printf("AHCI: Port %u interface error (SERR=0x%08X)\n", 
...
static int ahci_find_free_slot(ahci_port_t *port) {
    uint32_t slots = port->cmd_issued | ahci_port_read(port, AHCI_PxCI) | 
...
int ahci_read_async(ahci_port_t *port, uint64_t lba, uint32_t count,
    ...
    uint32_t buf_virt = 0xFA000000 + (slot * 0x10000);
    for (uint32_t i = 0; i < buf_pages; i++) {
...
void ahci_poll_completion(ahci_port_t *port) {
    uint32_t ci = ahci_port_read(port, AHCI_PxCI);
    uint32_t completed = port->cmd_issued & ~ci;
...
void ahci_irq_handler(void *regs) {
    ...
    uint32_t is = ahci_hba_read(g_ahci_controller.abar_virt, AHCI_IS);
...
void ahci_enable_interrupts(void) {
    ...
    uint32_t irq_line = pci_read_config(g_ahci_controller.pci_bus,
    ...
    uint32_t ghc = ahci_hba_read(g_ahci_controller.abar_virt, AHCI_GHC);
    ghc |= AHCI_GHC_IE;
    ...
    for (uint32_t i = 0; i < AHCI_MAX_PORTS; i++) {
        ahci_port_t *port = &g_ahci_controller.ports[i];
        if (port->present) {
            uint32_t ie = AHCI_PxIS_DHRS | AHCI_PxIS_PSS | AHCI_PxIS_DSS |
