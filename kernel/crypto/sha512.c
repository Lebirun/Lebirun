...
static void sha512_transform(sha512_ctx_t *ctx, const uint8_t data[128])
{
    ...
    for (i = 0; i < 80; i++) {
        t1 = h + EP164(e) + CH64(e, f, g) + K[i] + w[i];
...
{
    uint32_t i;
    uint64_t total_bits_lo, total_bits_hi;

    i = ctx->datalen;
    total_bits_lo = ctx->bitlen[0] + (uint64_t)ctx->datalen * 8;
    total_bits_hi = ctx->bitlen[1];
    if (total_bits_lo < ctx->bitlen[0]) {
        total_bits_hi++;
    }
    ...
    ctx->data[127] = (uint8_t)(total_bits_lo);
    ctx->data[126] = (uint8_t)(total_bits_lo >> 8);
    ctx->data[125] = (uint8_t)(total_bits_lo >> 16);
    ctx->data[124] = (uint8_t)(total_bits_lo >> 24);
    ctx->data[123] = (uint8_t)(total_bits_lo >> 32);
    ctx->data[122] = (uint8_t)(total_bits_lo >> 40);
    ctx->data[121] = (uint8_t)(total_bits_lo >> 48);
    ctx->data[120] = (uint8_t)(total_bits_lo >> 56);
    ctx->data[119] = (uint8_t)(total_bits_hi);
    ctx->data[118] = (uint8_t)(total_bits_hi >> 8);
    ctx->data[117] = (uint8_t)(total_bits_hi >> 16);
    ctx->data[116] = (uint8_t)(total_bits_hi >> 24);
    ctx->data[115] = (uint8_t)(total_bits_hi >> 32);
    ctx->data[114] = (uint8_t)(total_bits_hi >> 40);
    ctx->data[113] = (uint8_t)(total_bits_hi >> 48);
    ctx->data[112] = (uint8_t)(total_bits_hi >> 56);
    sha512_transform(ctx, ctx->data);
